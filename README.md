...
- **`src/core/`**：核心逻辑与系统配置
  - `main.cpp`：程序入口与主循环
  - `Config.h`：全局静态配置中心（所有可调参数集中管理）
  - `FrameQueue.h/.cpp`：线程安全的帧队列实现
  - `CoordinateCalculator.h/.cpp`：坐标计算与防抖状态机

- **`src/vision/`**：视觉算法模块
  - `CameraManager.h/.cpp`：双相机并行管理器（核心调度模块）
  - `Detector.h/.cpp`：YOLOv11 ONNX 推理器
  - `Tracker.h/.cpp`：目标跟踪器
  - `ContourProcessor.h/.cpp`：轮廓处理与纸张区域提取
  - `AutoLabeler.h/.cpp`：自动标注工具（用于数据采集）
...
系统采用 `Config.h` 作为唯一的参数源，实现了配置与代码的完全分离：

1. **全局唯一性**：所有可调参数均在 `Config.h` 中定义，禁止在其他文件中出现硬编码数值。
2. **分类管理**：参数按硬件、AI算法、传统视觉、系统四个维度分组，注释详尽。
3. **编译期常量**：使用 `inline constexpr` 定义，零运行时开销，类型安全。

### 多线程与并发控制

系统采用生产者-消费者模型，实现采集、推理、显示的流水线并行：

1. **独立线程**：每个相机（数字/豆子）拥有独立的处理线程，通过 `CameraManager` 统一调度。
2. **最新帧覆盖策略**：采用 `std::mutex` 保护帧缓存，读取时直接覆盖旧帧，彻底解决帧积压导致的延迟问题。
...
- **操作系统**：Windows 10/11（已测试），Linux（需调整相机索引与串口路径）
...
| `NUM_CLASSES` | `int` | 5 | 模型类别总数（必须与训练时一致） |
...
| `SERIAL_BAUD` | `int` | 115200 | 串口波特率（必须与下位机一致） |
...
│   ├── vision/                   # 视觉算法模块
│   │   ├── CameraManager.h/.cpp  # 双相机并行管理器
│   │   ├── Detector.h/.cpp       # YOLO 推理器
│   │   ├── Tracker.h/.cpp        # 目标跟踪器
│   │   ├── ContourProcessor.h/.cpp  # 轮廓处理器
...
└── dataset/                      # 数据集目录（用于自动标注）
```

## 运行说明

1. 连接两台 USB 工业相机，确认设备索引（Windows 可通过相机应用测试）。
